...
device = torch.device('cuda:9') if use_cuda else torch.device('cpu')
...
if __name__ == "__main__":
    ...
    weights1 = torch.ones(state.size()[0], device=device) 
    weights2 = torch.ones(state.size()[0], device=device) 
    T = args.T
    ...
    for t in range(T+1):
        ...
        if (t+1) % 1000 == 0:
            ...
            state, state_next, sub_state, sub_state_next, weights1, weights2 = state[idx,:], state_next[idx,:], sub_state[idx,:], sub_state_next[idx,:], weights1[idx], weights2[idx]
        ...
        state_batch,state_next_batch,sub_state_batch,sub_state_next_batch, weight1, weight2 = state[start:end], state_next[start:end], sub_state[start:end], sub_state_next[start:end], weights1[start:end], weights2[start:end]
        if use_cuda:
        ...
        losses_returned1, loss1 = net.loss_red(predicts1, state_next_batch[:, 0], [weight1, weight2], nll_red)
        predicts2, latent2, latentp2 = net.back_forward(sub_state_next_batch[:,0]) 
        losses_returned2, loss2 = net.loss_inverse_red(latentp2, latents1[-1:], [weight1, weight2], L1_red)
        ...
        if t > 0 and t % 10000 == 0:
            ...
            scale = latent.size()[2]
            # weight1
            log_density, _ = kde_density(latent[:,0])
            log_rho = - scale * torch.log(2.0*torch.from_numpy(np.array(L)))
            logp = log_rho - log_density
            weights1 = to_weights(logp) * state.size()[0]
            weights1 = weights1.to(device)
            # weight2
            log_density, _ = kde_density(latent[:,1])
            log_rho = - scale * torch.log(2.0*torch.from_numpy(np.array(L)))
            logp = log_rho - log_density
            weights2 = to_weights(logp) * state.size()[0]
        ...
        if  t % 10000 == 0:
            ...
            # [50, 2, 36, 9], [50, 2, 18*18], [50, 2, 18*18]
            eis = net.calc_EIs(sub_state_next_temp[:,0], latents[-1:], latent_ps)
