...
class Renorm_Dynamic(nn.Module):
    ...
    def forward_train(self, x,xp):
        if len(x.size())<=1:
            x = x.unsqueeze(0)
        
        s, z = self.encoding1(x)
    ...
    def decoding(self, s_next,de_noi_size=1):
        ...
        y,_ = self.flow.g(zz)
        return y
    def decoding1(self, s_next,de_noi_size=1):
        sz = self.sym_size - self.latent_size
        if sz>0:
            noise = distributions.MultivariateNormal(torch.zeros(sz), torch.eye(sz)).sample((s_next.size()[0], 1))
            noise = noise.to(self.device)*torch.tensor([de_noi_size*de_noi_size],device=device)
            #print(noise.size(), s_next.size(1))
            if s_next.size()[0]>1:
                noise = noise.squeeze(1)
            else:
    ...
    def decoding_train(self, s_next, x_next):
        sz = self.sym_size - self.latent_size
        x_next_encode,_=self.flow.f(x_next)
        z=x_next_encode[:, self.latent_size:]
        if sz>0:
            noise = z
            noise = noise.to(self.device)
            #print(noise.size(), s_next.size(1))
    ...
    def decoding2(self, s_all):
        sz = self.sym_size - self.latent_size
        s_next=s_all[:,:self.latent_size]
        if sz>0:
            noise = s_all[:,self.latent_size:]
            noise = noise.to(self.device)
            #print(noise.size(), s_next.size(1))
            if s_next.size()[0]>1:
                noise = noise.squeeze(1)
...
class Parellel_Renorm_Dynamic(nn.Module):
    def __init__(self, sym_size, latent_size, effect_size, cut_size, hidden_units,normalized_state,device,is_random=False,inv_size=2):
        #latent_size: input size
        #effect_size: scale, effective latent dynamics size
        ...
        self.inv_dynamics = nn.Sequential(nn.Linear(inv_size, hidden_units), nn.LeakyReLU(), 
                                 nn.Linear(hidden_units, hidden_units), nn.LeakyReLU(), 
                                 nn.Linear(hidden_units, inv_size))
        self.normalized_state=normalized_state
        self.is_random = is_random
        self.sigmas = torch.nn.parameter.Parameter(torch.rand(1, inv_size, device=self.device))
    def build_flow(self, input_size, hidden_units):
    ...
    def back(self,sp):
        predicts1_back=self.encoding(sp)
        predicts1_back_next=self.inv_dynamics(predicts1_back[-2])-predicts1_back[-2]
        return predicts1_back_next
    ...
    def encoding(self, x):
        ...
        for i,flow in enumerate(self.flows):
            ...
            ys.append(y)
        return ys
    def loss(self, predictions, real, w):
        losses = []
        ...
        for i, predict in enumerate(predictions):
            if i==1:
                loss=(MAE_raw(real, predict).mean(axis=1) * w).mean()
            else:
                loss = MAE(real, predict)  
            losses.append(loss)
            sum_loss += loss
